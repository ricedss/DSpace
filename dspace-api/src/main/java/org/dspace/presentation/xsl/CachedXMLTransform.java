...
public class CachedXMLTransform extends XMLTransform
{
    ...
    private Bundle findCacheBundle(Item item)
    {
        try
        {
            Bundle[] bundles = item.getBundles(CACHE_BUNDLE_NAME);
            if (bundles.length > 0)
            {
                return bundles[0];
            }
    ...
    {
        for (Bitstream bitstream : Arrays.asList(cacheBundle.getBitstreams()))
        {
    ...
    private void deleteCachedBitstream(Context context, Bundle cacheBundle, Bitstream cachedBitstream) throws AuthorizeException, SQLException, IOException
    {
        try
        {
            ...
            cacheBundle.removeBitstream(cachedBitstream);
            cacheBundle.update();
            context.commit();
        }
        finally
            ...
            String cachedName, InputStream bitsIn) throws AuthorizeException, SQLException, IOException
    {
        try
        {
            ...
            if (cacheBundle == null)
            {
                cacheBundle = item.createBundle(CACHE_BUNDLE_NAME);
            }

            assert(bitsIn.markSupported());
            bitsIn.mark(bitsIn.available()+1);
            Bitstream cacheBitstream = cacheBundle.createBitstream(bitsIn);
            if (bitsIn.markSupported())
            bitsIn.reset();

            // Now set the format and name of the bitstream
            cacheBitstream.setName(cachedName);
            cacheBitstream.setSource(makeCachedBitstreamSource(getName())
                    + String.valueOf(System.currentTimeMillis()));
            cacheBitstream.setDescription(sourceBitstream.getName()+": "+getDescription());
            BitstreamFormat bf = BitstreamFormat.findByShortDescription(context, "HTML");
            cacheBitstream.setFormat(bf);

            log.info(LogManager.getHeader(context, "cache_transformed_bitstream",
                    "bitstream_id=" + cacheBitstream.getID()));
            cacheBitstream.update();
            item.update();
            context.commit();
        }
        finally
            ...
            throws AuthorizeException, SQLException, IOException, JDOMException, TransformerException
    {
        ...
        if (cachedBitstream == null)
        {
            ...
            bitsIn = cachedBitstream.retrieve();
        }
