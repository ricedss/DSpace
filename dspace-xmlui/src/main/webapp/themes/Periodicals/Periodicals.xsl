                ...
                <div class="hiddenvalue">
                    <xsl:for-each select="key('issues-by-vol', @vol)">
                        <p>
                            ...
                            <a href="{$context_path}/handle/{$collection_handle}/search?{$query_string}=series:%22Volume%20{$volnum}%20Issue%20{@num}%22">
                                <!-- i18n: Browse Issue -->
    ...
    <xsl:template match="dim:dim" mode="issue">
        ...
        <xsl:if test="dim:field[@element='relation' and @qualifier='ispartofseries']">
            <tr class="ds-table-row">
                ...
                <td>
                    <a href="{$context_path}/handle/{$collection_handle}/search?{$query_string}=series%3A%28%22Volume+{$vol}%2C+Issue+{$num}%22+-%22Page%22%29">Issue <xsl:value-of select="$num"/></a>
                </td>
            </tr>
        </xsl:if>
    </xsl:template>
