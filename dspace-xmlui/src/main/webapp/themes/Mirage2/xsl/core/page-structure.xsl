...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="addJavascript">
        ...
        <!-- SWB adapted to:
          1) only deploy analytics if the server is "scholarship", not test or anything else, and
          2) deploy multiple codes for themes with 'google.extra' configured in their sitemap, in
             addition to the global 'google.analytics' from dspace.cfg: xmlui.google.analytics.key -->
        ...
        <xsl:if test="contains($host_name,'scholarship.rice.edu') and /dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='google'][@qualifier='analytics']">
            <script>
              ...
              <xsl:text>', '</xsl:text><xsl:value-of select="$host_name"/>
              ...
              <xsl:for-each select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='google'][@qualifier='extra']">
                  <xsl:text>ga('create', '</xsl:text><xsl:value-of select="."/>
                  <xsl:text>', '</xsl:text><xsl:value-of select="$host_name"/>
                  <xsl:text>', {'name':'theme'}); ga('theme.send', 'pageview');</xsl:text>
              </xsl:for-each>
