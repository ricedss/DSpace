...
<xsl:stylesheet
    ...
    <!-- Ying: For certain fields where the text is likely going to be the same, pre-fill some of the forms.
    ...
    <xsl:template match="dri:field[@type='text']" mode="normalField">
        <input>
            ...
            <xsl:attribute name="value">
                <xsl:choose>
                    <xsl:when test="@n='dc_contributor_publisher'">
                        <xsl:text>Houston, Tex.:Rice University</xsl:text>
                    </xsl:when>
		            <xsl:when test="@n='dc_publisher'">
                    ...
                    <xsl:when test="@n='dc_identifier_original'">
                        <xsl:text>LD4711 .R3k N48</xsl:text>
                    </xsl:when>
    ...
    <xsl:template match="dri:field[@type='textarea' and @n='dc_digitization_specifications']" mode="normalField">
        <textarea>
            ...
            <xsl:apply-templates />
            <i18n:text>Derivative files are OCR'd PDF for text</i18n:text>
        </textarea>
    </xsl:template>

    <!-- MMS: For the dc.digitization.specifications field, pre-fill it with the expected text. -->
    <xsl:template match="dri:field[@type='textarea' and @n='dc_description_abstract']" mode="normalField">
        <textarea>
            <xsl:call-template name="fieldAttributes"/>
            <xsl:call-template name="textAreaCols"/>
            <xsl:attribute name="rows">4</xsl:attribute>
            <xsl:apply-templates />
            <i18n:text>A Library newsletter to the Rice University community. Published twice a year (spring and fall)</i18n:text>
        </textarea>
    </xsl:template>
</xsl:stylesheet>
