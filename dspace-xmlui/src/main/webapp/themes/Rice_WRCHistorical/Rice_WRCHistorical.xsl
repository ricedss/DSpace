...
<xsl:stylesheet
            ...
            <xsl:attribute name="value">
                <xsl:choose>
                    ...
                    <xsl:when test="@n='dc_rights'">
                        <xsl:text>This work is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.</xsl:text>
                    </xsl:when>
    ...
    <xsl:template match="dri:field[@type='textarea' and @n='dc_rights_uri']" mode="normalField">
        <textarea>
            <xsl:call-template name="fieldAttributes"/>
            <xsl:call-template name="textAreaCols"/>
            <xsl:attribute name="rows">4</xsl:attribute>
            <xsl:apply-templates />
            <i18n:text>xmlui.Submission.submit.DescribeStep.hint.wrchistorical.rightsuri</i18n:text>
        </textarea>
    </xsl:template>
    ...
    <xsl:template name="buildFooter">
        ...
        <xsl:if test="contains($host_name,'scholarship.rice.edu')">
            ...
            <script type="text/javascript">
                var _gaq = _gaq || [];
                _gaq.push(['_setAccount', 'UA-29633606-1']);
