...
public class XMLServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        if (idString != null)
        {
            ...
            try
            {
                item = (Item) HandleManager.resolveToObject(context,
                        handle);
                ...
                for (int i = 0; (i < bundles.length) && bitstream == null; i++)
                {
                    Bitstream[] bitstreams = bundles[i].getBitstreams();

                    for (int k = 0; (k < bitstreams.length) && bitstream == null; k++)
                    {
        ...
        if (bitstream != null)
        {
            ...
            if (mimetype.equals("text/xml"))
            {
                ...
                Metadatum[] schemas = item.getDC("format", "xmlschema", Item.ANY);
                String schema = (schemas.length > 0)? schemas[0].value : "";
